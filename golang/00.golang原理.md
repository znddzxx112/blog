...
每个worker会等待一个信号，等到调用gcController.findRunnable才会真正执行
...
问题：标记有几种模式？三种。如果是一心一意执行标记，会把p本地Gouroutine放到全局队列，继续执行扫描。
...
从p的gcWork取出灰色对象，标记为黑色。如果依旧有可达对象，调用`greyobject`把可达对象放入p的gcWork中
...
1、 markroot函数扫描root根对象，都放入盗p的gcWork中

2、scanobject函数从gcWork取出对象，如果对象依旧有可达对象，再放入到gcWork中
...
原文链接：https://blog.csdn.net/u010853261/java/article/details/84790392
