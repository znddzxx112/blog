...
5、二叉树，平衡二叉树（红黑树）：高度平衡，左树小于右树
6、字典树
题目：给你100000个长度不超过10的单词。对于每一个单词，我们要判断他出没出现过，如果出现了，求第一次出现在第几个位置。
如果我们使用一般的方法，没查询一个单词都去遍历一遍，那么时间复杂度将为O(n^2),这对于100000这么大的数据是不能够接受的。

1、加载敏感词树，常驻内存。定时任务更新，加锁更新
type trieNode struct {
	value string
	map[string]*trieNode
	isStop bool
}
2、待检测的一段文本，在敏感词树进行查找，终止条件：当前node终止符号或者*trieNode为Null
```
- 算法问题
```
旅行家问题：从起点城市到终点城市。
