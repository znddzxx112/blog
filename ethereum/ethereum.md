...
1、比特币没有明确账户机制，一个私钥控制未花费一堆公钥地址btc数目之和。即UTXO之和。

2、以太坊的账户有明确的账户机制，一个私钥对应公钥对应固定账户，全局状态树上的一个节点，记录余额、nonce,code。

3、账户以太坊与比特币明显不一样地方。所以比特币要防止双花攻击(取决于矿工打包顺序，等待6个区块确认)，以太坊要防止重放攻击（通过nonce值）

4、以太坊账户160bits,20bytes,40位0~f表示。总共可以有2的160个账户，十分稀疏。

5、账户分为EOA外部账户，智能合约账户。账户地址与地址状态映射（记录余额、nonce,code）。

6、以太坊账户技术实现通过一颗modify MPT树，所有账户地址经过路径压缩，再把普通指针转换为哈希指针，形成根哈希。叶子节点（key, value）

value的管理，将nonce,balance,storage通过rlp编码

MPT特点：经过路径压缩的树。特点：有序，不会冲突，更新操作的局部性，访问内存次数减少。
...
1、收据与交易一一对应。收据树与交易树都采用MPT树，key为交易hash。

2、相比状态树，每个区块都有收据树和交易树。状态树是全局只有一颗树。

3、轻节点不存储交易列表，通过MPT树的merkle proof进行校验

4、每个收据都有布隆过滤器，整个区块记录了整个收据树布隆过滤器根hash

5、收据作用：每个交易执行后，记录交易结果。交易执行生成日志，保存在布隆过滤器中。
...
作用：解决某个元素是否在一个大数据结构中。

在ethereum中作用：判断某个事件是否产生。

原理：给大集合的每个元素计算一个hash值放在紧凑的小集合中

达到的效果：可能会误报，不会漏报。追求好的效果，减少元素的hash值碰撞。
...
1、整个以太坊是交易驱动的状态机。状态机：有具体的账户状态

2、整个比特币是交易驱动的状态机。状态机：UTXO，未花费的输出



#### GHOST

1、以太坊使用改良的GHOST，叔块不排资论辈。以太坊的共识协议建立在应用层。

2、作用：大幅度缩短出块事件后，临时性分叉常态化。让挖到区块但没有成为最长合法链，能得到大部分奖励。

3、叔块根据远近依次能拿到7/8，6/8，5/8 ～ 2/8

4、挖到以太坊区块奖励：1、block reward  2、gas fee 3、uncle reward

5、挖到btc区块奖励: 1、block reward 2、tx fee

6、以太坊没有减半说法，叔父区块的交易不被执行，也不检查区块的交易，只检查是否符合挖矿难度。

7、分叉的第一个区块才能得到uncle reward



#### 挖矿算法

1、以太坊目标是达到one cpu , one vote，设计出对ASIC不友好的算法。比如SCRYPT算法。因为轻节点存在还要兼顾difficult to solve, but easy to verify

2、ASIC矿机特点是多CPU，但是内存小，所以ASIC不友好的算法是大内存就好

3、数据结构[16Mcache  1G dataset],

3.1、先随即生成数据填满16M的cache

3.2、通过cache填充1G dataset

3.3、每隔3万块，修改cache和dataset数据集合的大小
...
pow比拼算力多寡，本质是比钱，pos本质是虚拟挖矿，能减少能耗，温室气体

pos是占有币越多，获得更多币，内部形成了闭环。
...
#### 以太坊2.0准备采用casper的权益证明

引入见证者，见证者会缴纳保证金，拥有投票权。

如果验证者投票能得到奖励，如果分叉二边下注，没收保证金，如果造假，要伙同见证者。
...
答：创建交易，接收地址要填写合约地址，data域填写调用函数或函数的编码值。data的前4字节就是函数标识符。

6、一个合约如何调用另一个合约函数？

答：1、直接调用。如果调用合约发生错误,本函数也出错

2、使用address类型的call。发起调用函数，继续执行

3、delegatecall()调用环境为本函数中



#### 先执行合约交易还是先挖矿？汽油费怎么扣除？

答：先执行合约交易，得到交易树和状态树的根hash再打包。此时本地可见。

其他接收区块的全节点都是独立验证，验证通过丢掉本地状态，再根据区块交易列表执行一遍。

gas费从账户先gasLimit*price扣除，多退



#### 区块链的安全性如何保障？

通过全节点的独立验证，独立执行，如果不验证则三颗树hash对不上。

网络广播的区块只有三棵树的hash和交易列表。
...
1、addr.transfer() addr.send():2300个gas发过去  call.value()把剩余gas全部发出去
...
原理：把eth发给合约，合约返回代币spiltDAO，代币可以进行投票和利益分配。

赎回eth：成立childDao小组织，当小组织只有一个账户时就是赎回。代币回收，发送eth

黑客发现合约漏洞，盗取10%全网币

以太坊官方先通过软分叉，但是失败

后来通过投票采取硬分叉，增加了chainID，防止重入攻击。最终导致结果产生了etc
