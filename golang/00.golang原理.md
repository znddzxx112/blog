...
export GOROOT_BOOTSTRAP=$HOME/local/go1.16
```
...
$ cd src
$ ./make.bash --no-clean
$ cp -r $HOME/.golang/go/bin/go* $HOME/.golang/go1.4/bin/
$ cd ../../go
$ git checkout -b release-branch.go1.16 origin/release-branch.go1.16
$ cd src
$ ./make.bash --no-clean
$ go version
```
...
`runtime·osinit(SB)` 获取到cpu核数、页大小64位为4KB
